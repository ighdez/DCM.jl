...
"""
function evaluate(expr::DCMExpression, data::DataFrame, params::AbstractDict, expanded_draws::AbstractDict, expanded_vars::AbstractDict)
    result = begin
        if expr isa DCMParameter
            N = nrow(data)
        ...
        elseif expr isa DCMSum
            left = evaluate(expr.left, data, params, expanded_draws, expanded_vars)
            right = evaluate(expr.right, data, params, expanded_draws, expanded_vars)
        ...
        elseif expr isa DCMExp
            arg = evaluate(expr.arg, data, params, expanded_draws, expanded_vars)
            T = eltype(arg)
        ...
        elseif expr isa DCMEqual
            left_val = evaluate(expr.left, data, params, expanded_draws, expanded_vars)
            T = eltype(left_val)
